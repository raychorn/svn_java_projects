...
class HTMLOutputStream extends FilterOutputStream implements DTDConstants {
     ...
     */
    void writeTag(Element elem, Attributes atts) {
	write('<');
	write(elem.getName());
    ...
    public void startTag(Element elem, Attributes atts) {
	// REMIND: REMOVE
	writeTag(elem, atts);
	write('\n');
    }
    ...
    public void endTag(Element elem) {
	// REMIND: REMOVE
	write('\n');
	write('<');
	write('/');
    ...
    public void emptyTag(Element elem, Attributes atts) {
	// REMIND: REMOVE
	writeTag(elem, atts);
    }
     ...
     * Output text.
     */
    public void text(byte data[], int off, int len) {
	if (plain == 0) {
	    for (int i = 0 ; i < len ; i++) {
