...
class DTD implements DTDConstants {
	...
	try {
	    props.load(new FileInputStream(System.getenv("HOME") + 
	...
	// Set firewall info
	net.www.http.HttpClient.firewallProxyHost = props.getProperty("firewallHost");
	net.www.http.HttpClient.firewallProxyPort = Integer.valueOf(props.getProperty("firewallPort")).intValue();


	// Parse the DTD
	DTD dtd = getDTD(argv[0]);
	dtd.print();
    }
}
